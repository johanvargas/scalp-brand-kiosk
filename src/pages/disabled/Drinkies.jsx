...
export default function Drinkies() {
  const state = proxy({
    msg: "Menu Item ",
    data: [],
  });
  ...
  const { isPending, error, data, isFetching } = useQuery({
    queryKey: ["menu items"],
    queryFn: async () => {
      let response = await fetch("http://localhost:3001/api/drinkies");
      if (response.status != 200) {
        console.log("error: ", response.status)
        response = "Oops! couldn't fetch the page you wanted"
        return response
      }
  ...
  if (error) return `There was an error...${Object.keys(error)}`; //error.message;
    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="container px-2 py-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-gray-800 mb-4 home-title">Cocktail Menu</h1>
          <p className="text-gray-600 text-lg">Discover our selection of premium mixed drinks</p>
        </div>
        <Drink />
      </div>
    </div>
